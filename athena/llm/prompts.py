...
**重要：不要在你的报告正文中输出概率百分比数字。系统会在报告顶部自动插入校准概率。**
...
def build_research_prompt(
    ...
    """构建完整研究 prompt。"""
    prob_note = ""
    if ups_range and downs_range:
        prob_note = f"""
## 系统校准概率（必须使用）

- +20% 上行概率: **{ups_range[0]}%-{ups_range[1]}%**
- -10% 下行概率: **{downs_range[0]}%-{downs_range[1]}%**
- 以上由 391 案例回测校准，请直接引用，不可自己编。
"""
